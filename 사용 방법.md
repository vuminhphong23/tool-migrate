...
### Workflow ví dụ
```
categories → authors → posts → comments
```
...
## 🔐 PHẦN 3 — ROLE, PERMISSION & FLOW MIGRATION

### Mục tiêu
Đảm bảo các **Roles**, **Policies**, **Permissions**, và **User Assignments** được migrate chính xác và an toàn.
...
### Thứ tự migrate
1. **Roles** → không có dependency  
2. **Policies** → không có dependency  
3. **Permissions** → phụ thuộc vào Policies  
4. **User Assignments / Flow** → sau khi Roles đã có  

---

### Thách thức thường gặp
- Mapping ID giữa Policies và Permissions.  
- Khác biệt tên collection, field giữa môi trường.  
- Rủi ro bảo mật (quá quyền / thiếu quyền).  
- Cấu trúc rule JSON phức tạp.
...
- Đếm và phân loại roles, policies, permissions.  
- Kiểm tra collections/fields tồn tại trên target.  
- Phát hiện xung đột, rủi ro bảo mật.

#### **2. Logic migrate thông minh**
- RoleMigration: preserve ID, skip admin roles, xử lý conflict.  
- PolicyMigration: validate, mapping collections/fields.  
- PermissionMigration: validate rules, skip invalid, transform mapping.

#### **3. Kiểm tra bảo mật**
- Đánh giá các thay đổi quyền admin.  
- So sánh new vs removed permissions.  
- Gắn nhãn risk: `over_permissive`, `under_permissive`, `admin_access`.

#### **4. Tích hợp giao diện**
- Giao diện chọn roles/policies/permissions.  
- Xem trước diff trước khi apply.  
- Theo dõi progress & logging chi tiết.
...
- ⚠️ Validate field và collection tồn tại trên target.  
